...
function startImportSequence() {
    // add listener
    chrome.runtime.onMessage.addListener(import_runtimeMessageListener);
...
function importNext(){
    ...
    if (curIdx >= organizedInvoiceList.length) {
        ...
        chrome.runtime.onMessage.removeListener(import_runtimeMessageListener);
...
function import_runtimeMessageListener(message, sender, sendResponse) {
